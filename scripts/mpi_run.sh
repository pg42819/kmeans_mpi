...
program=${KMEANS_PROGRAM:-${prog}}
#timeout_arg="--timeout 10 --report-state-on-timeout --get-stack-traces"
timeout_arg=""
#mpi_extras="${MPI_EXTRAS:-} --map-by ppr:${sockets}:socket"
mpi_extras="${MPI_EXTRAS:-} --map-by ppr:${sockets}:core"
...
command="mpirun -n ${processes} ${mpi_extras} ${KMEANS_BIN_DIR}/${program} --${debug_level} \
